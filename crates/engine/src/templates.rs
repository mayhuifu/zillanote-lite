...
- The transcript contains recognition errors. Product names, acronyms, numbers and units are often mis-heard or spelled out in words (\"A二百\" for \"A200\", \"三千二百万\" for \"3200万\", \"二百四十兆\" for \"240MHz\", \"seventy K\" for \"70K\"). Write every term, number and unit in its standard written form, the same way each time, and use the spellings from the list of known terms when one is given. Do not annotate doubts in the text. If a doubt changes the meaning of a decision, name it once under \"AI suggestions\". Never guess a person's name or a figure.
...
pub const PREVIOUS_SYSTEM_PROMPTS: &[&str] = &[SYSTEM_PROMPT_0_1, SYSTEM_PROMPT_0_2];

/// 0.2.0 to 0.2.2: the same as today's but for the examples of mis-heard terms, which were
/// taken from real meetings and are now made up.
const SYSTEM_PROMPT_0_2: &str = r#"You write meeting minutes from a transcript made by automatic speech recognition. The reader wants the outcome on one page: what was concluded and what happens next, not who said what when.
...
mod tests {
    ...
    fn earlier_defaults_are_kept_so_untouched_prompts_can_move_on() {
        ...
        // The 0.2 prompt differs from today's only in the examples of mis-heard terms.
        let today = DEFAULT_SYSTEM_PROMPT.replace("A二百", "X三百").replace("A200", "X300");
        let today = today.replace("三千二百万", "四千三百万").replace("3200万", "4300万");
        let today = today.replace("二百四十兆", "四百五十兆").replace("240MHz", "450MHz");
        let today = today.replace("seventy K", "ninety K").replace("70K", "90K");
        assert_eq!(PREVIOUS_SYSTEM_PROMPTS[1], today);
    }
}
